...
function reportDetail(r) {
  ...
  const diagnosis = db.prepare('SELECT * FROM diagnoses WHERE work_order_id = ?').get(r.work_order_id) || null;
  const workPerformed = db.prepare('SELECT * FROM work_performed WHERE work_order_id = ? ORDER BY created_at').all(r.work_order_id);
  ...
  const photos = db.prepare('SELECT * FROM attachments WHERE work_order_id = ? ORDER BY created_at').all(r.work_order_id)
...
async function getReportHandler(ctx) {
  const r = db.prepare('SELECT * FROM service_reports WHERE id = ?').get(ctx.params.id);
  if (!r) return sendJSON(ctx.res, 404, { error: 'Service report tidak ditemukan' });
  const wo = getWorkOrder(r.work_order_id);
...
async function approveReportHandler(ctx) {
  ...
  db.prepare("UPDATE service_reports SET status = 'APPROVED', approved_at = ?, approved_by = ?, updated_at = ? WHERE id = ?")
    .run(now(), ctx.user.id, now(), r.id);
  db.prepare("UPDATE work_orders SET status = 'APPROVED', updated_at = ? WHERE id = ?").run(now(), wo.id);
  if (t) {
    setTicketStatus(t, 'CLOSED', ctx.user, 'Laporan service disetujui');
  ...
  const partsTotal = partTotalForWorkOrder(wo.id);
  const invId = uid();
  const invNumber = nextNumber('INV');
  const issued = now();
...
async function rejectReportHandler(ctx) {
  ...
  const { note = '' } = ctx.body;
  const wo = getWorkOrder(r.work_order_id);
  db.prepare("UPDATE service_reports SET status = 'REJECTED', technician_note = ?, updated_at = ? WHERE id = ?")
...
async function resubmitReportHandler(ctx) {
  ...
  db.prepare("UPDATE service_reports SET status = 'SUBMITTED', summary = ?, technician_note = ?, version = version + 1, updated_at = ? WHERE id = ?")
...
async function analyticsHandler(ctx) {
  ...
  const ticketsByMonth = months.map((m) => ({
    ...m,
    count: db.prepare(`SELECT COUNT(*) AS c FROM tickets WHERE substr(created_at,1,7) = ? ${tf.sql}`).get(m.key, ...tf.params).c
  }));
  ...
  const byServiceType = db.prepare(`SELECT service_type AS label, COUNT(*) AS count FROM tickets WHERE 1=1 ${tf.sql} GROUP BY service_type ORDER BY count DESC`).all(...tf.params);
  const byPriority = db.prepare(`SELECT priority AS label, COUNT(*) AS count FROM tickets WHERE 1=1 ${tf.sql} GROUP BY priority ORDER BY count DESC`).all(...tf.params);
  const byEquipmentType = db.prepare(`SELECT COALESCE(NULLIF(equipment_type,''),'(terdaftar)') AS label, COUNT(*) AS count FROM tickets WHERE 1=1 ${tf.sql} GROUP BY label ORDER BY count DESC`).all(...tf.params);
  ...
  const totalTickets = db.prepare(`SELECT COUNT(*) AS c FROM tickets WHERE 1=1 ${tf.sql}`).get(...tf.params).c;
  const completedTickets = db.prepare(`SELECT COUNT(*) AS c FROM tickets WHERE status IN ('COMPLETED','CLOSED') ${tf.sql}`).get(...tf.params).c;
  const cancelledTickets = db.prepare(`SELECT COUNT(*) AS c FROM tickets WHERE status = 'CANCELLED' ${tf.sql}`).get(...tf.params).c;
  const openTickets = db.prepare(`SELECT COUNT(*) AS c FROM tickets WHERE status IN ('OPEN','REVIEWING','ASSIGNED','IN_PROGRESS') ${tf.sql}`).get(...tf.params).c;
