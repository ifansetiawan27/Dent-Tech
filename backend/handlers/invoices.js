...
function invoiceDetail(inv, user) {
  const parts = partTotalForWorkOrder(inv.work_order_id);
  ...
  const customer = db.prepare('SELECT id, code, name, address, city, phone, email FROM customers WHERE id = ?').get(inv.customer_id);
  const payments = db.prepare('SELECT * FROM payments WHERE invoice_id = ? ORDER BY paid_at').all(inv.id);
  const wo = inv.work_order_id ? db.prepare('SELECT * FROM work_orders WHERE id = ?').get(inv.work_order_id) : null;
  const ticket = inv.ticket_id ? db.prepare('SELECT number, problem, service_type FROM tickets WHERE id = ?').get(inv.ticket_id) : null;

  let evidence = { photos: [], checklist: null, diagnosis: null };
  if (wo) {
    const rawPhotos = db.prepare('SELECT * FROM attachments WHERE work_order_id = ? ORDER BY created_at').all(wo.id);
    ...
    evidence.checklist = buildChecklistState(wo);
    evidence.diagnosis = db.prepare('SELECT findings, root_cause, recommendation FROM diagnoses WHERE work_order_id = ? ORDER BY updated_at DESC LIMIT 1').get(wo.id) || null;
  }
...
async function listInvoicesHandler(ctx) {
  ...
  for (const inv of rows) {
    inv.totals = invoiceTotals(inv, partTotalForWorkOrder(inv.work_order_id));
  }
  sendJSON(ctx.res, 200, { invoices: rows });
}

async function getInvoiceHandler(ctx) {
  const inv = db.prepare('SELECT * FROM invoices WHERE id = ?').get(ctx.params.id);
  ...
  sendJSON(ctx.res, 200, invoiceDetail(inv, ctx.user));
}
...
async function updateInvoiceHandler(ctx) {
  ...
  if (labor_cost !== undefined) db.prepare('UPDATE invoices SET labor_cost = ? WHERE id = ?').run(Number(labor_cost), inv.id);
  if (discount !== undefined && Number(discount) >= 0) db.prepare('UPDATE invoices SET discount = ? WHERE id = ?').run(Number(discount), inv.id);
  if (tax_rate !== undefined && Number(tax_rate) >= 0) db.prepare('UPDATE invoices SET tax_rate = ? WHERE id = ?').run(Number(tax_rate), inv.id);
  if (due_at !== undefined) db.prepare('UPDATE invoices SET due_at = ? WHERE id = ?').run(due_at, inv.id);
...
async function payInvoiceHandler(ctx) {
  ...
  if (ctx.user.role === 'customer' && inv.customer_id !== ctx.user.customer_id) return sendJSON(ctx.res, 403, { error: 'Tidak memiliki akses' });
  if (inv.status === 'PAID') return sendJSON(ctx.res, 400, { error: 'Invoice sudah dibayar' });
  const totals = invoiceTotals(inv, partTotalForWorkOrder(inv.work_order_id));
  ...
  if (inv.ticket_id) {
    const t = getTicket(inv.ticket_id);
...
// ---------- Invoice settings (PPN / Non-PPN, default labor) ----------
async function getInvoiceSettingsHandler(ctx) {
  sendJSON(ctx.res, 200, {
    tax_mode: getSetting('invoice_tax_mode', 'PPN'),
    default_labor: Number(getSetting('invoice_default_labor', '500000'))
  });
}

async function updateInvoiceSettingsHandler(ctx) {
  const { tax_mode, default_labor } = ctx.body;
  if (tax_mode !== undefined) {
    if (!['PPN', 'NON_PPN'].includes(tax_mode)) return sendJSON(ctx.res, 400, { error: 'Mode PPN tidak valid' });
    setSetting('invoice_tax_mode', tax_mode);
  }
  if (default_labor !== undefined) {
    ...
    setSetting('invoice_default_labor', String(v));
  }
  audit(ctx.user, 'UPDATE', 'settings', 'invoice', 'Memperbarui pengaturan invoice', ctx.ip);
  sendJSON(ctx.res, 200, { ok: true, tax_mode: getSetting('invoice_tax_mode', 'PPN'), default_labor: Number(getSetting('invoice_default_labor', '500000')) });
}
...
async function createProformaHandler(ctx) {
  ...
  const existing = db.prepare("SELECT id, number FROM invoices WHERE work_order_id = ? AND type = 'PROFORMA' AND status != 'PAID'").get(wo.id);
  if (existing) return sendJSON(ctx.res, 409, { error: `Proforma ${existing.number} sudah ada untuk work order ini` });

  const labor = labor_cost !== undefined ? Number(labor_cost) : Number(getSetting('invoice_default_labor', '500000'));
  const taxRate = getSetting('invoice_tax_mode', 'PPN') === 'NON_PPN' ? 0 : 11;
  const invId = uid();
  const invNumber = nextNumber('PI');
  const issued = now();
  const due = localDate(new Date(Date.now() + (Number(due_days) || 14) * 24 * 3600 * 1000));
  db.prepare(`INSERT INTO invoices (id, number, ticket_id, work_order_id, customer_id, labor_cost, discount, tax_rate, status, type, issued_at, due_at, created_at)
  ...
  const totals = invoiceTotals(inv, partTotalForWorkOrder(wo.id));
  if (t) {
