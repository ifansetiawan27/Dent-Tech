...
async function listCustomersHandler(ctx) {
  ...
  const params = [];
  if (search) { sql += ' AND (name LIKE ? OR code LIKE ? OR city LIKE ?)'; params.push(`%${search}%`, `%${search}%`, `%${search}%`); }
  sql += ' ORDER BY name ASC';
  const customers = db.prepare(sql).all(...params);
  for (const c of customers) {
    c.equipment_count = db.prepare('SELECT COUNT(*) AS c FROM equipment WHERE customer_id = ?').get(c.id).c;
    c.open_tickets = db.prepare("SELECT COUNT(*) AS c FROM tickets WHERE customer_id = ? AND status NOT IN ('CLOSED','CANCELLED')").get(c.id).c;
    c.contacts = db.prepare('SELECT * FROM customer_contacts WHERE customer_id = ? ORDER BY is_primary DESC').all(c.id);
  }
...
async function createCustomerHandler(ctx) {
  ...
  const id = uid();
  const code = 'CUS-' + String(nextNumber('CUS').split('-')[2]);
  ...
  for (const ct of contacts) {
    if (!ct.name) continue;
    db.prepare('INSERT INTO customer_contacts (id, customer_id, name, role, phone, email, is_primary) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
async function getCustomerHandler(ctx) {
  ...
  c.contacts = db.prepare('SELECT * FROM customer_contacts WHERE customer_id = ? ORDER BY is_primary DESC').all(id);
  c.equipment = db.prepare('SELECT * FROM equipment WHERE customer_id = ? ORDER BY name').all(id);
  sendJSON(ctx.res, 200, { customer: c });
}

async function updateCustomerHandler(ctx) {
  const c = db.prepare('SELECT * FROM customers WHERE id = ?').get(ctx.params.id);
  if (!c) return sendJSON(ctx.res, 404, { error: 'Customer tidak ditemukan' });
  const fields = ['name', 'industry', 'phone', 'email', 'address', 'city', 'status'];
  for (const f of fields) {
    if (ctx.body[f] !== undefined) db.prepare(`UPDATE customers SET ${f} = ? WHERE id = ?`).run(ctx.body[f], c.id);
  }
  if (Array.isArray(ctx.body.contacts)) {
    db.prepare('DELETE FROM customer_contacts WHERE customer_id = ?').run(c.id);
    for (const ct of ctx.body.contacts) {
      if (!ct.name) continue;
      db.prepare('INSERT INTO customer_contacts (id, customer_id, name, role, phone, email, is_primary) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
async function deleteCustomerHandler(ctx) {
  const c = db.prepare('SELECT * FROM customers WHERE id = ?').get(ctx.params.id);
  if (!c) return sendJSON(ctx.res, 404, { error: 'Customer tidak ditemukan' });
  const tickets = db.prepare('SELECT COUNT(*) AS c FROM tickets WHERE customer_id = ?').get(c.id).c;
  if (tickets > 0) return sendJSON(ctx.res, 409, { error: 'Customer memiliki riwayat ticket dan tidak dapat dihapus' });
  db.prepare('DELETE FROM customer_contacts WHERE customer_id = ?').run(c.id);
  db.prepare('DELETE FROM equipment WHERE customer_id = ?').run(c.id);
  db.prepare('DELETE FROM users WHERE customer_id = ? AND role = ?').run(c.id, 'customer');
...
async function listEquipmentHandler(ctx) {
  ...
  if (search) where.push('(e.name LIKE ? OR e.serial_number LIKE ? OR e.model LIKE ?)'), params.push(`%${search}%`, `%${search}%`, `%${search}%`);
  if (where.length) sql += ' WHERE ' + where.join(' AND ');
  sql += ' ORDER BY e.created_at DESC';
  const equipment = db.prepare(sql).all(...params);
  for (const e of equipment) {
    e.service_count = db.prepare('SELECT COUNT(*) AS c FROM tickets WHERE equipment_id = ?').get(e.id).c;
    e.last_service = db.prepare("SELECT created_at FROM tickets WHERE equipment_id = ? ORDER BY created_at DESC LIMIT 1").get(e.id)?.created_at || null;
  }
...
async function getEquipmentHandler(ctx) {
  const e = db.prepare('SELECT e.*, c.name AS customer_name FROM equipment e JOIN customers c ON c.id = e.customer_id WHERE e.id = ?').get(ctx.params.id);
  ...
  e.service_history = db.prepare(histSql).all(e.id);
  sendJSON(ctx.res, 200, { equipment: e });
}

async function updateEquipmentHandler(ctx) {
  const e = db.prepare('SELECT * FROM equipment WHERE id = ?').get(ctx.params.id);
  if (!e) return sendJSON(ctx.res, 404, { error: 'Equipment tidak ditemukan' });
  const fields = ['name', 'category', 'model', 'serial_number', 'location', 'install_date', 'warranty_until', 'status'];
  for (const f of fields) {
    if (ctx.body[f] !== undefined) db.prepare(`UPDATE equipment SET ${f} = ? WHERE id = ?`).run(ctx.body[f], e.id);
  }
...
async function deleteEquipmentHandler(ctx) {
  const e = db.prepare('SELECT * FROM equipment WHERE id = ?').get(ctx.params.id);
  if (!e) return sendJSON(ctx.res, 404, { error: 'Equipment tidak ditemukan' });
  const tickets = db.prepare('SELECT COUNT(*) AS c FROM tickets WHERE equipment_id = ?').get(e.id).c;
...
async function listPartsHandler(ctx) {
  ...
  for (const p of parts) {
    p.used_total = db.prepare('SELECT COALESCE(SUM(qty),0) AS c FROM part_usages WHERE part_id = ?').get(p.id).c;
  }
...
async function createPartHandler(ctx) {
  ...
  const id = uid();
  const code = 'PRT-' + String(nextNumber('PRT').split('-')[2]);
...
async function updatePartHandler(ctx) {
  const p = db.prepare('SELECT * FROM parts WHERE id = ?').get(ctx.params.id);
  ...
  for (const f of fields) {
    if (ctx.body[f] !== undefined) db.prepare(`UPDATE parts SET ${f} = ? WHERE id = ?`).run(ctx.body[f], p.id);
  }
...
async function adjustStockHandler(ctx) {
  ...
  db.prepare('UPDATE parts SET stock = ? WHERE id = ?').run(newStock, p.id);
...
// ---------------- Checklist Templates ----------------
function templateWithItems(id) {
  const tpl = db.prepare('SELECT * FROM checklist_templates WHERE id = ?').get(id);
  if (!tpl) return null;
  tpl.items = db.prepare('SELECT * FROM checklist_template_items WHERE template_id = ? ORDER BY sort_order').all(id);
  return tpl;
}
...
async function listTemplatesHandler(ctx) {
  ...
  for (const t of templates) {
    t.item_count = db.prepare('SELECT COUNT(*) AS c FROM checklist_template_items WHERE template_id = ?').get(t.id).c;
  }
  sendJSON(ctx.res, 200, { templates });
}

async function getTemplateHandler(ctx) {
  const tpl = templateWithItems(ctx.params.id);
  if (!tpl) return sendJSON(ctx.res, 404, { error: 'Template tidak ditemukan' });
  sendJSON(ctx.res, 200, { template: tpl });
}

function saveTemplateItems(templateId, sections) {
  db.prepare('DELETE FROM checklist_template_items WHERE template_id = ?').run(templateId);
  let sort = 0;
  for (const sec of sections || []) {
    for (const item of sec.items || []) {
      if (!item.label) continue;
      db.prepare('INSERT INTO checklist_template_items (id, template_id, section, label, required, sort_order) VALUES (?, ?, ?, ?, ?, ?)')
...
async function createTemplateHandler(ctx) {
  ...
  const id = uid();
  db.prepare('INSERT INTO checklist_templates (id, name, service_type, equipment_category, version, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(id, name, service_type, equipment_category, version, 'ACTIVE', now());
  saveTemplateItems(id, sections);
...
async function updateTemplateHandler(ctx) {
  const tpl = db.prepare('SELECT * FROM checklist_templates WHERE id = ?').get(ctx.params.id);
  if (!tpl) return sendJSON(ctx.res, 404, { error: 'Template tidak ditemukan' });
  const { name, service_type, equipment_category, version, status, sections } = ctx.body;
  if (name !== undefined) db.prepare('UPDATE checklist_templates SET name = ? WHERE id = ?').run(name, tpl.id);
  if (service_type !== undefined) db.prepare('UPDATE checklist_templates SET service_type = ? WHERE id = ?').run(service_type, tpl.id);
  if (equipment_category !== undefined) db.prepare('UPDATE checklist_templates SET equipment_category = ? WHERE id = ?').run(equipment_category, tpl.id);
  if (version !== undefined) db.prepare('UPDATE checklist_templates SET version = ? WHERE id = ?').run(version, tpl.id);
  if (status !== undefined) db.prepare('UPDATE checklist_templates SET status = ? WHERE id = ?').run(status, tpl.id);
  if (Array.isArray(sections)) saveTemplateItems(tpl.id, sections);
